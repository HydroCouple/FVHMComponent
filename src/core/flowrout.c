...
static void   initLinks(Project* project, int routingModel);                                     //(5.1.008)
...
//
{
	...
	initNodes(project);
	initLinks(project, routingModel);                                                   //(5.1.008)
}
...
//
{
    ...
    vFixed = project->Node[i].oldVolume + 
             0.5 * (project->Node[i].oldNetInflow + project->Node[i].inflow - 
                    project->Node[i].outflow) * dt;                                     //(5.1.007)
    ...
    while ( iter < MAXITER && !stopped )
    {
		...
		v2 = vFixed - 0.5 * getStorageOutflow(project, i, j, links, dt) * dt;           //(5.1.007)
...
//
{
    ...
    newNetInflow = project->Node[j].inflow - project->Node[j].outflow - project->Node[j].losses;          //(5.1.007)
...
//
{
    ...
    if ( project->Link[j].type == CONDUIT )
    {
        ...
        else 
        {
			...
			q -= link_getLossRate(project, j, q, tStep);                                //(5.1.008)
            if ( q < 0.0 ) q = 0.0;

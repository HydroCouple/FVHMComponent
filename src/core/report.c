...
#define WRITE(y,x) (report_writeLine((y),(x)))
#define LINE_10 "----------"
#define LINE_12 "------------"
#define LINE_51 \
"---------------------------------------------------"
...
//extern char   project->ErrString[81];           // defined in ERROR.C
...
//
{
    ...
    {
        fprintf(project->Frpt.file, FMT20, ctime(&project->SysTime));
        time(&endTime);
        fprintf(project->Frpt.file, FMT20a, ctime(&endTime));
        elapsedTime = difftime(endTime, project->SysTime);
...
//
{
    ...
    time(&project->SysTime);                    // Save starting wall clock time
}
...
//
{
    ...
    if ( project->Nobjects[LINK] > 0 )
    {
        ...
        fprintf(project->Frpt.file, "\n  Number of Threads ........ %d", project->NumThreads);   //(5.1.008)
		fprintf(project->Frpt.file, "\n  Head Tolerance ........... %.6f ",
            project->HeadTol*UCF(project,LENGTH));                                              //(5.1.008)
...
"\n  -------------------------------------------------------------------------------\n");
    ...
    else
    {
        ...
        fprintf(project->Frpt.file, "  %-10s %-11s  %-11s  %5d min    %6ld     %6ld     %6ld\n",
...
////  Following code segment added to release 5.1.008.  ////                   //(5.1.008)
    if ( totals->runon > 0.0 )
    {
        fprintf(project->Frpt.file, "\n  Outfall Runon ............%14.3f%14.3f",
			totals->runon * UCF(project, LENGTH) * UCF(project, LANDAREA),
			totals->runon / totalArea * UCF(project, RAINDEPTH));
    }
////
...
////  Following code segment added to release 5.1.008.  ////                   //(5.1.008)
    if ( totals->drains > 0.0 )
    {
        fprintf(project->Frpt.file, "\n  LID Drainage .............%14.3f%14.3f",
			totals->drains * UCF(project, LENGTH) * UCF(project, LANDAREA),
			totals->drains / totalArea * UCF(project, RAINDEPTH));
    }
...
////
    ...
    for (p = p1; p <= p2; p++)
    {
        fprintf(project->Frpt.file, "%14.3f", QualTotals[p].seepLoss);
    }
////
...
//
{
    ...
    for (i=0; i<nMaxStats; i++)
    {
        ...
        if ( maxCourantCrit[i].objType == NODE )
             fprintf(project->Frpt.file, "\n  Node %s", project->Node[j].ID);
        else fprintf(project->Frpt.file, "\n  Link %s", project->Link[j].ID);
        ...
        if ( project->Subcatch[j].rptFlag == TRUE )
        {
            ...
            for ( period = 1; period <= project->Nperiods; period++ )
            {
                ...
                fprintf(project->Frpt.file, "\n  %11s %8s %10.3f%10.3f%10.4f",
                    theDate, theTime, project->SubcatchResults[SUBCATCH_RAINFALL],
                    project->SubcatchResults[SUBCATCH_EVAP]/24.0 +
                    project->SubcatchResults[SUBCATCH_INFIL],
                    project->SubcatchResults[SUBCATCH_RUNOFF]);
                if ( hasSnowmelt )
                    fprintf(project->Frpt.file, "  %10.3f",
                        project->SubcatchResults[SUBCATCH_SNOWDEPTH]);
                if ( hasGwater )
                    fprintf(project->Frpt.file, "%10.3f%10.4f",
                        project->SubcatchResults[SUBCATCH_GW_ELEV],
                        project->SubcatchResults[SUBCATCH_GW_FLOW]);
                if ( hasQuality )
                    for (p = 0; p < project->Nobjects[POLLUT]; p++)
                        fprintf(project->Frpt.file, "%10.3f",
                            project->SubcatchResults[SUBCATCH_WASHOFF+p]);
            }
            WRITE(project, "");
            k++;
        }
...
//
{
    ...
    if ( hasSnowmelt ) fprintf(project->Frpt.file, "  Snow Depth");
        ...
        if ( project->Node[j].rptFlag == TRUE )
        {
            ...
            for ( period = 1; period <= project->Nperiods; period++ )
            {
                ...
                fprintf(project->Frpt.file, "\n  %11s %8s  %9.3f %9.3f %9.3f %9.3f",
                    theDate, theTime, project->NodeResults[NODE_INFLOW],
                    project->NodeResults[NODE_OVERFLOW], project->NodeResults[NODE_DEPTH],
                    project->NodeResults[NODE_HEAD]);
                if ( !project->IgnoreQuality ) for (p = 0; p < project->Nobjects[POLLUT]; p++)
                    fprintf(project->Frpt.file, " %9.3f", project->NodeResults[NODE_QUAL + p]);
            }
            WRITE(project, "");
            k++;
        }
    ...
    for (j = 0; j < project->Nobjects[LINK]; j++)
    {
        if ( project->Link[j].rptFlag == TRUE )
        {
            ...
            for ( period = 1; period <= project->Nperiods; period++ )
            {
                ...
                fprintf(project->Frpt.file, "\n  %11s %8s  %9.3f %9.3f %9.3f %9.3f",
                    theDate, theTime, project->LinkResults[LINK_FLOW],
                    project->LinkResults[LINK_VELOCITY], project->LinkResults[LINK_DEPTH],
                    project->LinkResults[LINK_CAPACITY]);
                if ( !project->IgnoreQuality ) for (p = 0; p < project->Nobjects[POLLUT]; p++)
                    fprintf(project->Frpt.file, " %9.3f", project->LinkResults[LINK_QUAL + p]);
            }
            WRITE(project, "");
            k++;
        }
...
//
{
    if ( project->Frpt.file )
    {
        report_writeErrorMsg(project,k, project->ErrString);

...
////  Local Variables
////-----------------------------------------------------------------------------
//THorton*   project->HortInfil = NULL;
...
static double grnampt_getUnsatInfil(Project* project, TGrnAmpt *infil, double tstep,
              double irate, double depth, int modelType);                      //(5.1.010)
static double grnampt_getSatInfil(Project* project, TGrnAmpt *infil, double tstep,
...
//
{
    ...
    case HORTON:
    case MOD_HORTON:
        project->HortInfil = (THorton *) calloc(subcatchCount, sizeof(THorton));
        if ( project->HortInfil == NULL ) project->ErrorCode = ERR_MEMORY;
        break;
    case GREEN_AMPT:
    case MOD_GREEN_AMPT:                                                       //(5.1.010)
        project->GAInfil = (TGrnAmpt *) calloc(subcatchCount, sizeof(TGrnAmpt));
        if ( project->GAInfil == NULL ) project->ErrorCode = ERR_MEMORY;
        break;
    case CURVE_NUMBER:
        project->CNInfil = (TCurveNum *) calloc(subcatchCount, sizeof(TCurveNum));
        if ( project->CNInfil == NULL ) project->ErrorCode = ERR_MEMORY;
        break;
...
//
{
    FREE(project->HortInfil);
    FREE(project->GAInfil);
    FREE(project->CNInfil);
}
...
//     subcatch  p1  p2 ...
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   status = horton_setParams(project,&project->HortInfil[j], x);
                         break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:                                                     //(5.1.010)
                         status = grnampt_setParams(project,&project->GAInfil[j], x);
                         break;
      case CURVE_NUMBER: status = curvenum_setParams(&project->CNInfil[j], x);
                         break;
      default:           status = TRUE;
    }
...
void infil_initState(Project* project, int j, int m)
//
...
//
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   horton_initState(&project->HortInfil[j]);   break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:                                                     //(5.1.010)
                         grnampt_initState(&project->GAInfil[j]);    break;
      case CURVE_NUMBER: curvenum_initState(&project->CNInfil[j]);   break;
    }
}
...
//
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   horton_getState(&project->HortInfil[j], x); break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:                                                     //(5.1.010)
                         grnampt_getState(&project->GAInfil[j],x);   break;
      case CURVE_NUMBER: curvenum_getState(&project->CNInfil[j], x); break;
    }
}
...
//
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   horton_setState(&project->HortInfil[j], x); break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:                                                     //(5.1.010)
                         grnampt_setState(&project->GAInfil[j],x);   break;
      case CURVE_NUMBER: curvenum_setState(&project->CNInfil[j], x); break;
    }
}
...
//
{
    ...
    {
      case HORTON:
          return horton_getInfil(project,&project->HortInfil[j], tstep, rainfall+runon, depth);

      case MOD_HORTON:
          return modHorton_getInfil(project,&project->HortInfil[j], tstep, rainfall+runon,
                                    depth);

      case GREEN_AMPT:
      case MOD_GREEN_AMPT:                                                     //(5.1.010)
        return grnampt_getInfil(project, &project->GAInfil[j], tstep, rainfall+runon, depth, m); //(5.1.010)

      case CURVE_NUMBER:
        depth += runon / tstep;
        return curvenum_getInfil(project,&project->CNInfil[j], tstep, rainfall, depth);
...
//
{
    ...
    double FF, FF1, r;
    double f0   = infil->f0 * project->Adjust.hydconFactor;                             //(5.1.008)
    double fmin = infil->fmin * project->Adjust.hydconFactor;                           //(5.1.008)
...
//
{
    ...
    if ( infil->Sat ) return grnampt_getSatInfil(project,infil, tstep, irate, depth);
    else return grnampt_getUnsatInfil(project, infil, tstep, irate, depth, modelType);
}
...
double grnampt_getUnsatInfil(Project* project, TGrnAmpt *infil, double tstep, double irate,
...
//
{
    ...
    double ks = infil->Ks * project->Adjust.hydconFactor;                               //(5.1.008)
    ...
    infil->T = 5400.0 / infil->Lu / project->Evap.recoveryFactor;
    ...
    {
        infil->Sat = TRUE;
        return grnampt_getSatInfil(project, infil, tstep, irate, depth);
    }
...
double grnampt_getSatInfil(Project* project, TGrnAmpt *infil, double tstep, double irate,
    double depth)
//
//  Input:   infil = ptr. to Green-Ampt infiltration object
//           tstep =  runoff time step (sec),

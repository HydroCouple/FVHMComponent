...
//extern double          project->MaxOutfallFlow;
...
void    writeGroundwater(Project* project);                                                //(5.1.008)
...
//
{
    ...
    if ( project->Nobjects[SUBCATCH] > 0 )
    {
             ...
             (project->Nobjects[AQUIFER] > 0  && !project->IgnoreGwater) )
        {
            ...
            if ( !project->IgnoreGwater ) writeGroundwater(project);                           //(5.1.008)
...
void    writeGroundwater(Project* project)
{
    int i, j;
    int count = 0;
    double totalSeconds = project->NewRunoffTime / 1000.;
    double x[9];

    if ( project->Nobjects[SUBCATCH] == 0 ) return;
    for ( j = 0; j < project->Nobjects[SUBCATCH]; j++ )
    {
        if ( project->Subcatch[j].groundwater != NULL ) count++;
    }
    if ( count == 0 ) return;

	WRITE(project, "");
	WRITE(project, "*******************");
	WRITE(project, "Groundwater Summary");
	WRITE(project, "*******************");
	WRITE(project, "");
    fprintf(project->Frpt.file,
...
"\n  -----------------------------------------------------------------------------------------------------");
    ...
    for ( j = 0; j < project->Nobjects[SUBCATCH]; j++ )
    {
        if ( project->Subcatch[j].area == 0.0 || project->Subcatch[j].groundwater == NULL ) continue;
        fprintf(project->Frpt.file, "\n  %-20s", project->Subcatch[j].ID);
        x[0] = project->Subcatch[j].groundwater->stats.infil * UCF(project,RAINDEPTH);
		x[1] = project->Subcatch[j].groundwater->stats.evap * UCF(project, RAINDEPTH);
		x[2] = project->Subcatch[j].groundwater->stats.deepFlow * UCF(project, RAINDEPTH);
		x[3] = project->Subcatch[j].groundwater->stats.latFlow * UCF(project, RAINDEPTH);
		x[4] = project->Subcatch[j].groundwater->stats.maxFlow * UCF(project, FLOW) * project->Subcatch[j].area;
        x[5] = project->Subcatch[j].groundwater->stats.avgUpperMoist / totalSeconds;
		x[6] = project->Subcatch[j].groundwater->stats.avgWaterTable * UCF(project, LENGTH) /
               totalSeconds;
        x[7] = project->Subcatch[j].groundwater->stats.finalUpperMoist;
		x[8] = project->Subcatch[j].groundwater->stats.finalWaterTable * UCF(project, LENGTH);
        for (i = 0; i < 9; i++) fprintf(project->Frpt.file, " %8.2f", x[i]);
    }
	WRITE(project, "");
}
...
"\n  ---------------------------------------------------------------------------------");
    ...
    for ( j = 0; j < project->Nobjects[NODE]; j++ )
    {
        ...
        fprintf(project->Frpt.file, "%7.2f  %7.2f  %7.2f  %4d  %02d:%02d  %10.2f",
			...
			days, hrs, mins, project->NodeStats[j].maxRptDepth * UCF(project, LENGTH));
    }
	WRITE(project, "");
}
...
"\n  Node                 Type           %3s      %3s  days hr:min    %8s    %8s     Percent",
        ...
        VolUnitsWords[project->UnitSystem]);                                            //(5.1.009)
    fprintf(project->Frpt.file,
"\n  -------------------------------------------------------------------------------------------------");
...
"\n  --------------------------------------------------------------------------------------------------");
        ...
        for ( j = 0; j < project->Nobjects[NODE]; j++ )
        {
            ...
            if ( addedVol > 0.0 )
            {
                ...
                pctSeepLoss = project->StorageStats[k].exfilLosses / addedVol * 100.0;  //(5.1.007)
            }

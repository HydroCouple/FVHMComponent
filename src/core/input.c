...
////  Shared variables
////-----------------------------------------------------------------------------
//static char *project->Tok[MAXTOKS];             // String tokens from line of input
//static int  project->Ntokens;                   // Number of tokens in line of input
//static int  project->Mobjects[MAX_OBJ_TYPES];   // Working number of objects of each type
//static int  project->Mnodes[MAX_NODE_TYPES];    // Working number of node objects
//static int  project->Mlinks[MAX_LINK_TYPES];    // Working number of link objects
...
//
{
	...
	while (fgets(line, MAXLINE, project->Finp.file) != NULL)
	{
		...
		if (sect == s_OPTION) errcode = readOption(project, line);
		else if (sect >= 0)   errcode = addObject(project, sect, tok);
...
//
{
	...
	case s_ADJUST:                                                           //(5.1.007)
		return climate_readAdjustments(project, project->Tok, project->Ntokens);                          //(5.1.007)
...
//           in shared variable project->Tok[].
//
